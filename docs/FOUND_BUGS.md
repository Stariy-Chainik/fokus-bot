# Найденные баги и их статус

Замеченные по ходу поведение-сохраняющего рефакторинга дефекты. Открытые пункты не исправлять
без отдельного согласования; устранённые сохранены как история решений.

## B1. proxy_record_go не проверял адресата разрешения — ✅ УСТРАНЁН (спецроли убраны)
- **Было:** любой педагог, отправив `proxy_record_go:TCH-XXXX`, начинал запись за этого педагога без одобрения — хендлер не проверял факт/адресата одобрения.
- **Устранено (2026-07):** прокси-механизм Клецовой удалён вместе со спецролями — хендлеры
  `proxy_record` / `proxy_approve` / `proxy_deny` / `proxy_record_go` вырезаны из
  `record_lesson/entry.py`. Запись за педагога осталась только у админа (независимый флоу
  `admin:record_lesson` → `admin_rl_tch:`, гейт `is_admin`). Уязвимый путь больше не существует.

## B2. В хлебных крошках wizard'а тип «shared» показывается сырым словом
- **Где:** [record_lesson/_base.py](../bot/handlers/teacher/record_lesson/_base.py) — `_KIND_LABEL` (нет ключа `shared`) + `_header()`.
- **Симптом:** при записи разового совместного занятия в заголовке экранов выбора показывается «shared» вместо русской подписи (у group/pair/soloist — «Группа/Пара/Соло»).
- **Почему баг:** `_KIND_LABEL.get(kind, kind)`-семантика подставляет сам ключ, словарь не пополнили при добавлении типа shared.
- **Репро:** запись занятия → тип «Разовое совместное» → на экране длительности/выбора в шапке виден англ. «shared».

## B3. Мёртвая ветка pair_from_soloists и перекрытый cb_noop — ✅ УДАЛЕНА (2026-07)
- **Была:** недостижимая цепочка «пара из солистов» (флаг ставился только в `False`) —
  `_show_pair_soloists_in_group`, состояние `picking_pair_soloists`, пара хендлеров
  `pso_toggle`/`pso_confirm` в `record_lesson/pair.py`; плюс дубль `cb_noop` в `soloist.py`,
  перекрытый ранним `common_router`.
- **Удалена** с подтверждением пользователя: недостижимые хендлеры, состояние и флаг вычищены.
  `kb_pair_from_soloists` жив — используется shared-флоу.

## B4. Webhook ЮКассы не верифицирует запрос (безопасность) — ✅ ИСПРАВЛЕНО
- **Где:** [bot/handlers/client/payments.py](../bot/handlers/client/payments.py) — `make_yookassa_webhook_handler`.
- **Симптом (был):** любой, кто знает URL `/yookassa-webhook`, мог POST-запросом с телом
  `{"event":"payment.succeeded","object":{"metadata":{"student_id":"STU-…","period_month":"YYYY-MM"}}}`
  перевести все счета периода в PAID без оплаты.
- **Почему баг:** тело запроса принималось на веру — ни IP-allowlist ЮКассы, ни перепроверки
  статуса платежа через API ЮКассы.
- **Исправлено (2026-07):** `process_yookassa_event` берёт из тела только `object.id` и
  перепроверяет платёж напрямую у API ЮКассы (`Payment.find_one`); статус/metadata/сумма —
  из ответа API. Подтверждение — только при реальном `succeeded`; сетевая ошибка проверки →
  HTTP 500 (ЮКасса ретраит). Покрыто тестами [tests/test_yookassa_webhook.py](../tests/test_yookassa_webhook.py).
- **Остаток (в веб-версии):** хранение внешнего `payment_id` для сверки/ручной перепроверки —
  модель `ExternalPayment` в [MINIAPP_BUILD.md §6.1](MINIAPP_BUILD.md).

## B5. История оплат и «Должники» по-разному считают «есть неоплаченное» — 🟡 ОТКРЫТ (аудит 2026-09-13)
- **Где:** [admin/payment_history.py](../bot/handlers/admin/payment_history.py) — `cb_payhist_group` (список учеников группы).
- **Симптом:** ученик помечен ⏳ в истории оплат, но отсутствует на экране «⚠️ Должники».
- **Почему:** история берёт **все** строки `student_period_payments` со статусом не-PAID и суммой > 0
  без учёта `DEBTORS_SINCE_PERIOD`, а «Должники» отсекают месяцы до этого периода.
- **Репро:** ученик с pending-строкой за 2026-05 при `DEBTORS_SINCE_PERIOD=2026-07`.
- Возможно, так и задумано (история — не долг); решить при выносе расчёта в сервис (план, шаг 3.2).

## B6. Mini App считает остаток мимо ledger — 🟡 ОТКРЫТ (аудит 2026-09-13)
- **Где:** [bot/api/miniapp.py](../bot/api/miniapp.py) — `bills()` (стр. 66–80).
- **Симптом:** остаток по педагогу = `total − paid_sums` вручную; `PaymentService.ledger_for` не вызывается —
  строка-остаток в листе не создаётся/не синхронизируется при просмотре из Mini App, переплата не показывается.
- **Почему:** дубль формулы вместо переиспользования `ledger_for`/`unpaid_for` (как в Telegram и MAX).
- **Риск:** расхождение между фронтами после изменения правил в `payment_ledger`.

## B7. Гость в PER_VISIT-группе всегда по полному тарифу — 🟡 ОТКРЫТ (аудит 2026-09-13)
- **Где:** [teacher/my_lessons/guests.py](../bot/handlers/teacher/my_lessons/guests.py) — стр. 110–113.
- **Симптом:** ученик с тарифом SHORT, добавленный в сохранённое групповое занятие кнопкой «добавить»,
  получает `price_full`; при записи занятия тот же ученик получил бы `price_short`
  (`utils/attendees.build_group_attendees_csv` учитывает `student.group_tier`).
- **Почему:** цена гостя считается в хендлере отдельной формулой без тарифа.
- **Репро:** детсадовская группа ЮБ/БП, ученик SHORT → «➕ добавить» в занятие → в `attendees` 850 вместо цены short.

## B8. Экран «📅 Месяцы членства» падает с NameError — ✅ ИСПРАВЛЕН (2026-09-13)
- **Где:** [admin/branches/joined.py](../bot/handlers/admin/branches/joined.py) — `_render_joined`, переменная `membership`.
- **Симптом:** нажатие «📅 Месяцы членства» в карточке абонементной группы (и подтверждение `gjdo:`/`gldo:`)
  завершается `NameError: name 'membership' is not defined` — экран не открывается.
- **Почему:** при выносе рендера в `_render_joined` потерялась строка
  `membership = await student_group_repo.get_membership_map()`.
- **Зафиксировано:** `tests/test_known_bugs.py::test_b8_joined_screen_raises_name_error`; ruff (F821) и mypy
  показывают эту находку как единственную оставшуюся.
- **Исправлено:** `membership = await student_group_repo.get_membership_map()` перед сборкой кнопок;
  тест переписан на golden-снимок экрана (`tests/golden/joined_screen.txt`).
