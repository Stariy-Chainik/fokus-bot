...
## Статус выполнения (2026-09-13, этапы B–D)

| Шаг | Статус | Коммиты / примечание |
|---|---|---|
| 0.1–0.3 pyproject, ruff+mypy, механические находки | ✅ | 259 → 0 находок ruff, 1317 → 0 mypy (после фикса B8) |
| 0.4 / 1.x характеризующие тесты, фейки, golden | ✅ | `tests/fakes.py`, 49 снимков, +46 тестов |
| 2.1 пагинация | ✅ | `utils/paging.py`, `nav_row` |
| 2.2 callback-датаклассы | ✅ топ-6 файлов | 254 → 199 `split(":")`; остальные — по мере правок |
| 2.3 N+1 | ✅ хендлеры + сервисы | см. I4 (что осталось) |
| 2.4 состав группы | ✅ | `services/rosters.group_members`, 14 копий |
| 2.5 notify_safely | ✅ | 3 сайта (остальные `except Exception` — не уведомления) |
| 3.1 фильтры ролей | ✅ | 240 хендлеров, `test_guards_static` |
| 3.2 доменная логика из хендлеров | частично | сделано: занятия родителя (деньги), гость в занятии, должники, «с 25-го»; **не сделано:** история оплат (суммы), расшифровка выплаты (группировка), `ClientService.ensure_client`, инвойсы в helpers |
