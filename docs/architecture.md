...
> ⚠️ **УСТАРЕЛО (снимок до рефакторинга, апрель 2026).** Этот документ описывает раннюю структуру
> и **не отражает** рефакторинг P4/P5: сервисный слой (`StudentService`, `StudentRequestService` и др.),
> разбивку god-файлов на пакеты (`admin/students/`, `admin/branches/`, `teacher/record_lesson/`),
> вынос спецконфига в `bot/staff.py`, экран «Должники». Оставлен как исторический контекст и концептуальная
> схема слоёв. **Актуальный источник истины — [../CLAUDE.md](../CLAUDE.md)** (реализация) и
> [BUSINESS_RULES.md](BUSINESS_RULES.md) (правила домена).
