...
class PaymentService:
    ...
    async def get_or_create_invoice(self, student: Student, period_month: str) -> StudentPeriodPayment:
        """У одного ученика за один месяц только одна запись в student_period_payments."""
        existing = await self._payment_repo.get_by_student_and_period(student.student_id, period_month)
        if existing:
            return existing

        billing_rows = await self._billing_repo.get_by_student_and_period(student.student_id, period_month)
        total = sum(b.amount for b in billing_rows)
        ...
        logger.info(
            "Создан счёт %s для student=%s период=%s сумма=%d",
            payment_id, student.student_id, period_month, total,
        )
        return payment
    ...
    async def confirm_payment(self, payment_id: str, confirmed_by_tg_id: int) -> bool:
        ...
        2. Проставляет payment_id во все billing-строки ученика за период
        """
        payment = next(
        ...
        if ok:
            ...
            logger.info("Счёт %s подтверждён, обновлено billing-строк: %d", payment_id, updated)
        return ok
