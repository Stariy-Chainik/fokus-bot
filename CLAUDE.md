...
(`p.home`), счета по месяцам (`p.bills`) и сам счёт (`p.bill`): позиции (педагог / абонемент) и занятия внутри них
отмечаются галочками, «К оплате» пересчитывается на лету, по умолчанию отмечено всё неоплаченное. `POST /pay`
