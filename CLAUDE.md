...
bot/handlers/
  ...
  admin/
    ...
    bills.py             # student invoices, send to parent (no submission check)
    profit.py            # school revenue (Выручка) vs salary summary
    branches.py          # branches, groups, teacher↔group assignment, bulk bill send
  ...
  client/
    ...
    my_bills.py          # invoices by month + payment flow + receipt upload
...
|---|---|
| `LessonService` | Create/delete lessons; period-lock check with `bypass_period_lock=True` for admins; duplicates not blocked for groups, blocked for solo |
| `BillingService` | Pure functions: `calc_earned()` and `build_billing_rows()` |
| `PaymentService` | Invoice computation; `get_or_create_invoices_for_student_period()`; `confirm_period()`; `create_yookassa_payment()` — does **not** check submission status |
...
| `CloudKassirService` | Sends fiscal receipt to client phone after admin confirms payment (`send_income_receipt`) |

### Utils

- `bot/utils/attendees.py` — parse/serialize `lesson.attendees` (CSV with optional duration:amount per student)
- `bot/utils/bill_format.py` — shared `build_bill_text()` used by both admin and teacher bill UIs
- `bot/utils/dates.py` — period/date display helpers
- `bot/utils/ids.py` — generates next sequential ID (`TCH-0001`, `STU-0042`, …)
- `bot/utils/lesson_stats.py` — aggregations for stats screens
...
Receipt upload uses FSM `ReceiptStates.waiting_for_receipt` (`bot/states/client_states.py`). On receipt: admins receive photo/document with «✅ Подтвердить оплату» button. Admin confirms → `confirm_period()` → all PENDING invoices for that (student, period) → PAID. If `CLOUDKASSIR_PUBLIC_ID` is set and the linked client has a phone, `CloudKassirService.send_income_receipt()` fires a fiscal receipt for the paid amount.
...
| Bills | `admin:bills` | Invoices per student per period; send to parent (no submission check) |
| Выручка | `admin:profit` | School revenue vs salary summary |
| Branches/Groups | `admin:branches` | CRUD branches, groups, billing modes, prices; bulk bill send for a group |
...
| **💰 Bills for own groups** | `teacher:bills` | Only for teachers in `BILLING_TEACHERS` (Контарева TCH-0009). Period → group → student card / bulk send. Scope strictly limited to teacher's own groups (`tb:g/s/snd/all` callbacks) |
...
- **Period submission**: once submitted, the teacher can no longer edit/delete lessons in that period. **Admins (and `bypass_period_lock=True` callers) can both create AND delete** lessons in submitted periods — `LessonService.create()` and `LessonService.delete()` both accept `bypass_period_lock`. The `kb_lesson_detail` keyboard renders the «🗑 Удалить занятие» button for admins even when locked (`is_admin=True` flag). Teachers can submit a period only from the **25th of the month**. Admins reopen the whole period via teacher card → «🔓 Открыть период» (deletes the submission row entirely).
- **Admin bills bypass submission**: admins (and teachers in `BILLING_TEACHERS`) can issue/send a bill to a parent at any moment — there is no «period not submitted» blocker anymore. Bill amounts are recomputed from current lessons. Caveat: lessons added AFTER an invoice was marked PAID are not auto-billed; either reopen the period or create a second invoice manually.
...
- **Duplicate guard**: групповые занятия не блокируются по дублям — одна группа может заниматься дважды в день (разные потоки/смены). Соло-занятия (ровно 1 ученик) блокируются по `teacher+student+date` через `individual_lesson_exists`.
...
**Контарева Елизавета (TCH-0009)** — teacher with billing access. Her menu has an extra «💰 Счета учеников» button (handlers in `bot/handlers/teacher/billing.py`). Scope is strictly her own groups: every callback verifies `_can_bill(user)`, `_is_my_group(...)`, and `_is_student_in_group(...)`. Bill content is the same as the admin one (combines lessons of all teachers for that student) — she sees the «full» bill, not only her own lessons. Whitelist defined as `BILLING_TEACHERS = {"TCH-0009"}` in `bot/keyboards/teacher.py`. Add a teacher_id to that set to grant the same access.
