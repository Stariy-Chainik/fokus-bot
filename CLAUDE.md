...
(`p.home`), счета по месяцам и разбивка счёта с отметкой ✅/⬜ у каждого занятия (`p.bills`, `p.bill`), выбор
занятий перед оплатой (`p.pick` → `PaymentService.set_payment_intent`), занятия месяца (`p.lessons`) и дневник
