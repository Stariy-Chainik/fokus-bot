...
> **Прогрев кеша.** `_cache_warmer` (в `bot/__main__.py`, раз в 240 с при TTL кеша 300 с) фоном **принудительно**
> перечитывает все листы из `_WARM_REPOS` через `BaseRepository.refresh()` (обычный `get_all()` при живом кеше в
> таблицу не ходит — прогрев через него ничего не обновлял, и 37 % времени листы были протухшими). Без прогрева
> первый запрос после простоя читает Google Sheets по-настоящему — 8–10 секунд, и Mini App успевает отвалиться по
> таймауту, а родитель жмёт «оплатить» повторно. С тёплым кешем сводка админа — ~150 мс; каждое чтение листа из
> Google — 0,3–0,7 с, поэтому новый лист сразу добавляйте в `_WARM_REPOS`. Первый прогрев после рестарта — ~6 с,
> в это окно запросы ещё читают таблицу сами.
...
«Финансы → Зарплаты → педагог» и «Выплатить зарплату → педагог»: у каждой строки-занятия подпись — группа
(и сколько пришло) или ученики, тап открывает карточку занятия (`admin_finance._described_lines`; сам
`SalaryService` имён не знает и отдаёт `label=""`).
...
свои занятия журналом по дням (день/месяц, фильтр «группы / индивидуальные», итог дня и месяца, карточка, удаление; у педагога с прямой оплатой вместо нуля видна сумма родителя — см. `DIRECT_PAY_TEACHER_IDS`), запись занятия общим с админом мастером
...
- **SHORT/FULL tiers** (`StudentGroupTier`): only for kindergarten groups (ЮБ/БП); all others have one price. Короткий тариф — свойство **группы** (`PER_VISIT` и `price_short > 0`, `student_service.has_short_tariff`), а флаг `students.group_tier` — лишь выбор ученика внутри такой группы. Нет ни одной группы с коротким тарифом → блока тарифа нет ни в карточке бота (`StudentCard.tariff_group`), ни в кабинете (`groups[].hasShort`), `toggle_tier` отвечает `NO_SHORT_TARIFF`; в занятии флаг SHORT без короткой цены в группе не применяется (`build_group_attendees_csv`). Экран биллинга группы и форма в кабинете показывают короткий тариф только когда он задан. Решение владельца 25.09.2026: у «ЮБ сад ХГ» (GRP-0008) короткого тарифа нет.
- **Group billing modes**: `NONE` (free, attendance not billed), `PER_VISIT` (each attended lesson billed at group price), `SUBSCRIPTION` (абонемент: фиксированная `price_full` ₽/мес с каждого ученика группы, **независимо от числа занятий**; начисляется **каждый месяц с первого занятия группы, каникулы тоже, кроме июля/августа** (летом — только за месяц с ≥1 занятием; `subscription_billable_months`, правило 2026-09-08); ключ начисления в счетах/долгах — `SUB:{group_id}`, подпись в счёте — просто «Абонемент» (название группы не влезало); см. `PaymentService._subscription_bills_for_student`). Начисляется только за месяцы членства (`joined_period` ≤ месяц < `left_period`) — учитывается в счетах, `compute_debt_map` и `subscription_revenue_breakdown`. Выход из абонементной группы не удаляет строку, а ставит `left_period` ([bot/services/membership.py](bot/services/membership.py): `leave_group`, `leave_options` — «с этого месяца» / «со следующего»); во всех трёх местах удаления (админ: карточка ученика и карточка группы, педагог: своя группа) показывается выбор месяца. UI правок — «📅 Месяцы членства» в карточке абонементной группы, callbacks `group_joined:` / `gjset:` / `gjdo:` / `gldo:`. В карточке группы кабинета ушедшие вынесены в свёрнутый блок «Ушли · N» (в счётчик состава не входят): у них не ✖ — он бы лишь заново проставил месяц ухода, — а «↩ Вернуть» (`PUT members {leftPeriod: ""}`) и 📅; строка не удаляется, потому что на ней держатся начисления и оплаты прошлых месяцев. Цена переопределяется на конкретный месяц для ученика или всей группы (лист `subscription_overrides`, приоритет: ученик/месяц → постоянное правило ученика (`period_month="*"`) → группа/месяц → `price_full`; `0` = не начислять; UI — «Биллинг ученикам» группы, callbacks `subovr:*`). Смена базовой цены действует **только вперёд**: админ выбирает месяц начала действия (`subeff:`), прошлые активные месяцы автоматически фиксируются старой ценой (или `0` при первом включении) — `PaymentService.pin_subscription_history`.
...
Optional — payments:
- `PARENT_BILLS_SINCE_PERIOD` — с какого месяца родитель видит **что-либо** в кабинете (по умолчанию `2026-09`, когда бот для родителей заработал): более ранние месяцы не попадают ни в счета (список, «📜 Оплаченные месяцы», сводка «Мои дети», `bill/{sid}/{ym}` → 404), ни в «Занятия» и «Дневник» (`?ym=` раньше границы → пусто, чипов месяцев нет; `/me.historySince`). Апрель–август 2026 закрыты оптом 09.09.2026 (546 строк, 116 учеников) — это архив школы, родителю не показывается. Данные не меняются — это только видимость. Та же граница в Telegram-боте и MAX (`parent_views.history_hidden` / `visible_periods`): список счетов без «📆 Другие месяцы», старый `client_bill:` / `cl_month:` / `cl_date:` / `cldiary:` отвечают alert'ом «показывается с сентября 2026», в дневнике нет кнопки «Прошлый месяц», календарь занятий назад не листается.
...
- `TEACHER_PERIOD_SUBMIT_ENABLED` — сдача периода педагогом (по умолчанию **выключена**, решение владельца 25.09.2026): нет кнопки «📤 Сдать период» в боте и в кабинете, `POST /api/teacher/submit` → 403 `disabled`, на сводке педагога нет замка и напоминаний «не сдан», в списке педагогов у админа нет 🟢/🔴. Уже сданные месяцы остаются закрытыми (замок на правку занятий действует), открыть их может админ через карточку педагога. Правило «с 25-го» и весь механизм сохранены — включается одной переменной.
...
- `DIRECT_PAY_TEACHER_IDS` — teacher_id через запятую (на проде: `TCH-0002` Клецова): **индивидуальные** занятия этих педагогов родители оплачивают педагогу напрямую — `build_billing_rows` не начисляет их (счета/долги/прибыль), `calc_earned` = 0. В счёте родителя такой педагог показан как все — раздел «Оплачивается педагогу напрямую» с занятиями и суммами, с пометкой «в „К оплате“ не входит, школа эти занятия не отслеживает»; в «К оплате», начисления школы и долги они не идут. Группы педагога — как обычно. Одна точка расчёта на все фронты — `PaymentService.direct_pay_rows()` → `DirectPayRow` ([payment_ledger.py](bot/services/payment_ledger.py)): кабинет отдаёт их строками `DIRECT:{tid}` (`rest=0`, галочки выбора нет), бот и MAX — блоком «Педагог: … — оплата напрямую» в конце счёта (`render_bill_detail(..., direct_by_student)`, `BillDetail.direct_total` в суммы счёта не входит).
  **В кабинете самого педагога** (решение владельца 24.09.2026) такие занятия не выглядят бесплатными: в журнале и карточке занятия вместо начисления школы показана сумма родителя («платит родитель», приглушённым) и аренда зала, в «Зарплате» нулевые строки свёрнуты в одну («Индивидуальные — прямая оплата, N занятий») и добавлен блок «Прямая оплата» — сколько должны родители по каждому ученику и сколько аренды перечислить школе. Считает API: `is_direct` / `direct_amount` / `_direct_summary` в [bot/api/teacher.py](bot/api/teacher.py) (`lessons`, `lessons/{id}`, `home`, `stats.direct`).
...
- `STUDENT_LESSON_RATES` — персональная цена ученика у педагога вместо `rate_for_student`: `TCH-XXXX:STU-XXXX:ставка[:с какого месяца]` (₽ за 45 мин, месяц `YYYY-MM` включительно; без месяца — за всё время). Зарплату педагога **не меняет** (`calc_earned` не трогает), действует только в счетах/долгах/прибыли через `billing_service.student_rate`. Месяц нужен, чтобы не пересчитывать уже оплаченные периоды. В паре каждый платит половину **своей** цены, поэтому сумма урока может отличаться от `rate_for_student`. На проде: `TCH-0005:STU-0019:3500:2026-09,TCH-0005:STU-0031:3500:2026-09` — Прудникова Дарья и Таня ProAm у Никишина, 3500 ₽ за 45 мин с сентября 2026 (решение владельца 24.09.2026).
