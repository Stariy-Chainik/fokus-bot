...
(педагог / абонемент) отмечаются галочками, занятия внутри позиции свёрнуты и раскрываются тапом, «К оплате»
пересчитывается на лету, по умолчанию отмечено всё неоплаченное. «Занятия» (`p.lessons`) — журнал посещений
по дням без сумм и кнопок: деньги живут только в счетах. `POST /pay`
