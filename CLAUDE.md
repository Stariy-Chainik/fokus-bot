...
| `StudentGroupRepository` | `student_groups` | Many-to-many student ↔ group (join table) + `joined_period` (кол. 3, `YYYY-MM`) — месяц вступления: абонемент начисляется **только с него**; пусто = «с начала группы». `add()` ставит текущий месяц, `set_joined_period()` правит задним числом, `get_joined_map()` → `{(sid, gid): period}` |
...
| `setup_joined_period.py [--apply]` | Идемпотентно добавляет `student_groups.joined_period` и проставляет существующим строкам первый месяц занятий их группы (поведение счётов не меняется). |
