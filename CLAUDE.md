...
пересчитывается на лету, по умолчанию отмечено всё неоплаченное. «Занятия» (`p.lessons`) — посещаемость: сводка месяца
(сколько занятий, часов, у кого), календарь месяца с точками по занятиям (цвет — группа/индивидуальное; тап по дню
оставляет в журнале только его, повторный тап или «✕ Весь месяц» возвращает месяц) и журнал по дням без сумм и кнопок: деньги живут только в счетах. `POST /pay`
...
- `DIRECT_PAY_TEACHER_IDS` — teacher_id через запятую (на проде: `TCH-0002` Клецова): **индивидуальные** занятия этих педагогов родители оплачивают педагогу напрямую — `build_billing_rows` не начисляет их (счета/долги/прибыль), `calc_earned` = 0. Родителю такие занятия видны в «Занятиях» с пометкой «оплата педагогу напрямую» и суммой (`build_billing_rows(..., include_direct=True)` — только для показа), внизу блок «Педагогу напрямую · N ₽» с оговоркой, что школа эти оплаты не отслеживает; в счёт, «К оплате» и долги они не попадают. Группы педагога — как обычно.
