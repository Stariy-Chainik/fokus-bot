...
Optional — payments:
- `PARENT_BILLS_SINCE_PERIOD` — с какого месяца родитель видит **что-либо** в кабинете (по умолчанию `2026-09`, когда бот для родителей заработал): более ранние месяцы не попадают ни в счета (список, «📜 Оплаченные месяцы», сводка «Мои дети», `bill/{sid}/{ym}` → 404), ни в «Занятия» и «Дневник» (`?ym=` раньше границы → пусто, чипов месяцев нет; `/me.historySince`). Апрель–август 2026 закрыты оптом 09.09.2026 (546 строк, 116 учеников) — это архив школы, родителю не показывается. Данные не меняются — это только видимость. Telegram-бот и MAX этой границы пока не знают.
