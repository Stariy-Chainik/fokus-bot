...
Optional — payments:
- `PARENT_BILLS_SINCE_PERIOD` — с какого месяца родитель видит счета в кабинете (по умолчанию `2026-09`): более ранние месяцы не попадают ни в список, ни в «📜 Оплаченные месяцы», ни в сводку «Мои дети». Данные не меняются — это только видимость для родителя.
