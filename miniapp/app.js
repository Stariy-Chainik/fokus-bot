...
/* ── API ─────────────────────────────────────────────────────────────── */
/* Роль задаёт и префикс API, и набор вкладок: администратор (a.*), педагог (t.*),
   родитель (p.*) или спортсмен (s.*). */
let ROLE = 'admin';
...
SCREENS['a.group'] = async ({ id }) => {
  ...
  const memberRow = (m, gone = false) => `<div class="cell static"><span class="lead">${initials(m.name)}</span><button class="cell nolead" style="padding:0;border:0;display:block;text-align:left" data-go="a.student" data-p='${esc(JSON.stringify({ id: m.id }))}'><div class="t">${esc(m.name)}</div><div class="s">${g.mode === 'subscription' ? `с ${m.joined ? monthLabel(m.joined).toLowerCase() : 'начала'}${m.left ? ` · ушёл с ${monthLabel(m.left).toLowerCase()}` : ''}` : (m.hasParent ? '' : 'без родителя')}</div></button><span class="r">${g.mode === 'subscription' ? `<button class="chip" style="padding:2px 8px" data-go="a.member" data-p='${esc(JSON.stringify({ gid: id, sid: m.id, name: m.name, joined: m.joined, left: m.left }))}'>📅</button> ` : ''}${gone
    ? `<button class="chip" style="padding:2px 8px" data-act="memberReturn" data-p='${esc(JSON.stringify({ gid: id, sid: m.id, name: m.name }))}'>↩ Вернуть</button>`
...
SCREENS['a.billing'] = async ({ id }) => {
  ...
  return { title: 'Биллинг ученикам', html: `
    ...
    ${ui.mode === 'per_visit' ? `${field('b-full', `Цена за посещение (${g.durationFull} мин), ₽`, g.priceFull || '', 'inputmode="numeric"')}${g.durationShort > 0 ? field('b-short', `Короткое занятие (${g.durationShort} мин), ₽ — только детсадовские группы`, g.priceShort || '', 'inputmode="numeric" placeholder="0 — нет"') : ''}<p class="hint">Цена попадает в новые занятия; уже отмеченные хранят свою.</p>` : ''}
...
Object.assign(ACT, {
  ...
  memberReturn: async ({ gid, sid, name }) => { try { await api(`/groups/${gid}/members/${sid}`, { method: 'PUT', body: { leftPeriod: '' } }); render(); toast(`${name} снова в группе`); } catch (e) { toast(errText(e)); } },
  ...
  tierToggle: async ({ id }) => { try { const r = await api(`/students/${id}/tier`, { method: 'POST' }); render(); toast(r.tier === 'short' ? 'Тариф: короткое занятие' : 'Тариф: полное занятие'); } catch (e) { toast(e.code === 'no_per_visit_group' ? 'Тариф есть только в группах «по посещению»' : e.code === 'no_short_tariff' ? 'В группах ученика нет короткого тарифа' : errText(e)); } },
...
(async () => {
  // роль подбираем по доступу: администратор → педагог → родитель → спортсмен
  let last = null;
  for (const role of ['admin', 'teacher', 'parent', 'athlete']) {
    ROLE = role;
